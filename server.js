...
function chooseWinner(){const alive=state.seats.filter(p=>p&&!p.eliminated&&p.balance>=ELIMINATION),targets=alive.filter(p=>p.balance>=WIN_TARGET).sort((a,b)=>b.balance-a.balance);return targets[0]||(state.started&&state.round>0&&alive.length===1?alive[0]:null)}
...
io.on("connection",socket=>{
 ...
 socket.on("join",({nickname,token},done=()=>{})=>{nickname=clean(nickname);if(!nickname)return done({ok:false,error:"닉네임을 입력하세요"});let p=state.seats.find(x=>x&&x.token===token)||state.seats.find(x=>x&&x.nickname===nickname&&!x.connected);if(p){p.connected=true;p.socketId=socket.id;clients.set(socket.id,p.id);emit();return done({ok:true,token:p.token,id:p.id,seat:state.seats.findIndex(x=>x?.id===p.id),reconnected:true})}if(state.started)return done({ok:false,error:"게임 시작 후에는 중간 참가할 수 없습니다"});const empty=state.seats.findIndex(x=>!x);if(empty<0)return done({ok:false,error:"10개 좌석이 모두 찼습니다"});if(state.seats.some(x=>x?.nickname===nickname))return done({ok:false,error:"이미 사용 중인 닉네임입니다"});p={id:crypto.randomUUID(),token:crypto.randomUUID(),nickname,balance:BUY_IN,connected:true,ready:false,eliminated:false,bet:emptyBet(),lastPay:0,socketId:socket.id};state.seats[empty]=p;clients.set(socket.id,p.id);done({ok:true,token:p.token,id:p.id,seat:empty});emit()});
